...
export default function HomePage() {
    ...
    const [selectedRoute, setSelectedRoute] = useState<{ name: string, id: number }>({ name: 'Select Route', id: 0 });
    const [selectedRetailer, setSelectedRetailer] = useState<{ retailer_name: string, id: number }>({ retailer_name: 'Select Retailer', id: 0 });
    const [selectedProductCategory, setSelectedProductCategory] = useState<{ category_name: string, id: number }>({ category_name: 'Select Product Category', id: 0 });
    ...
    const [selectedProduct, setSelectedProduct] = useState<CartProduct | null>(null); // Selected Product State
    ...
    const [totalAdditionalDiscountAmount, setTotalAdditionalDiscountAmount] = useState(0);
    const [selectedVisitType, setSelectedVisitType] = useState<string>('');
    const [showVisitPopup, setShowVisitPopup] = useState<boolean>(false);
    const visitType = ["Outlet closed", "Stock Available", "Shopkeeper Absent", "Fund Crisis", "Disagree"];
    ...
    useEffect(() => {
        ...
        const selectedProductListIsAvailable = localStorage.getItem('selectedProductList');

        if (routeName && routeId) setSelectedRoute({ name: routeName, id: parseInt(routeId) });
        if (retailerName && retailerId) setSelectedRetailer({ retailer_name: retailerName, id: parseInt(retailerId) });
        if (categoryName && categoryId) setSelectedProductCategory({ category_name: categoryName, id: parseInt(categoryId) });
    ...
    const handleDeleteProduct = (productId: number) => {
        const updatedList = selectedProductList.filter((product) => product.id !== productId);
        setSelectedProductList(updatedList);
        localStorage.setItem('selectedProductList', JSON.stringify(updatedList));
    };
        ...
        selectedProductList.forEach(product => {
            ...
            if(product.offer){
                ...
                if(product.offer?.additional_discount_amount){
                    additional_discount_amount += Number(product.offer?.additional_discount_amount);
                }
            }
        });
    ...
    const handleSaveOrder = async () => {
        if (selectedProductList) {
            ...
            for (let i=0; i<selectedProductList.length; i++) {
                const totalAmount = Number(selectedProductList[i].quantity) * Number(selectedProductList[i].price);
                let discount_amount: number = 0
                let is_sample: number = 0
                let is_gift: number = 0
                if(selectedProductList[i].offer){
                    discount_amount = Number(selectedProductList[i].offer?.discount_amount) + Number(selectedProductList[i].offer?.additional_discount_amount);
        ...
        <div className="flex flex-col items-center justify-start min-h-screen p-4 bg-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-3xl">
                ...
                <div className="relative w-full max-w-md">
                    ...
                    <div
                        onClick={() => router.push(`/retailer-list?route_id=${selectedRoute.id}`)}
                        className="w-full p-3 border border-gray-300 rounded-lg cursor-pointer flex justify-between items-center bg-white hover:border-gray-400 transition duration-200"
                    >
                        <span className="text-gray-700 text-sm truncate">
                            {selectedRetailer.retailer_name}{' '}
                            <span className="text-gray-500 text-xs">(ID: {selectedRetailer.id})</span>
                        </span>
                        <svg
                            className="w-5 h-5 text-gray-400"
                            fill="none"
...

                {
                    selectedRoute.id && selectedRetailer.id ?
                        <div className="flex gap-4 w-full">
                            ...
                            {/* Add Product Button */}
                            <button
                                onClick={() => router.push(`/product-list?category_id=${selectedProductCategory.id}`)}
            ...
            {selectedProductList.length > 0 && (
                <div className="w-full max-w-6xl mt-3 mx-auto">
                    {/* Header */}
                    <div
                        className="bg-gradient-to-r from-purple-100 to-blue-100 p-2 rounded-xl shadow-xl max-w-md mx-auto transition transform hover:scale-105 hover:shadow-2xl mb-3">
                        <div className="flex justify-between items-center text-gray-700">
                            <span className="font-extrabold text-xs tracking-wider text-gray-800">Order Date:</span>
                            <span className="text-xs text-gray-500">{currentDate}</span>
                        </div>
                        <div className="flex justify-between items-center text-gray-700">
                            <span className="font-extrabold text-xs tracking-wider text-gray-800">Total Qty:</span>
                        ...
                        </div>
                        {
                            totalDiscount || totalAdditionalDiscountAmount ?
                                <div className="flex justify-between items-center text-gray-700">
                                    <span
                                        className="font-extrabold text-xs tracking-wider text-red-800">Total Discount:</span>
                                    <span
                                        className="text-xs text-red-600">৳{Number(totalDiscount + totalAdditionalDiscountAmount).toFixed(2)}</span>
                                </div>
                                : null
                        }
                        <div className="flex justify-between items-center text-gray-700">
                            <span className="font-extrabold text-xs tracking-wider text-gray-800">Total Amount:</span>
                            <span
                                className="text-xs text-green-600">৳{Number(totalAmount - (totalDiscount + totalAdditionalDiscountAmount)).toFixed(2)}</span>
                        </div>
                    </div>
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-gray-700">
                                        <span
                                            className="text-gray-900 font-bold">৳{Number(product.price).toLocaleString('en-BD', {minimumFractionDigits: 2})}</span>
                                        ×
                                        <span className="text-gray-900 font-bold">{product.quantity}</span>
                                        =
                                        {product.offer?.discount_amount || product.offer?.additional_discount_amount ?
                                            <span
                                                className="text-gray-900 font-bold">৳{(Number(product.price) * Number(product.quantity)).toLocaleString('en-BD', {minimumFractionDigits: 2})}</span>
                                            :
                                            < span
                                                className="text-green-700 font-bold">৳{(Number(product.price) * Number(product.quantity)).toLocaleString('en-BD', {minimumFractionDigits: 2})}</span>
                                        }
                                        ...
                                        {product.offer?.discount_amount || product.offer?.additional_discount_amount ? (
                                            <>
                                                -
                                                <span
                                                    className="text-red-600 font-bold">৳{Number(Number(product.offer.discount_amount) + product.offer.additional_discount_amount).toLocaleString('en-BD', {minimumFractionDigits: 2})}</span>
                                                =
                                                <span
                                                    className="text-green-700 font-bold">৳{((Number(product.price) * Number(product.quantity)) - (Number(product.offer.discount_amount) + Number(product.offer.additional_discount_amount))).toLocaleString('en-BD', {minimumFractionDigits: 2})}</span>
                                            </>
                                        ) : null}
                                    </div>
                                    {
                                        product.offer?.sample_id ? (
                                            <p className="text-xs text-gray-600 mb-1 mt-1 p-2 bg-gradient-to-r from-blue-100 to-blue-300 rounded-lg shadow-md">
                                                <span
                                                    className="font-medium text-gray-700">Sample Product:</span> {product.offer?.sample_product_name} |
                                                Qty: {product.offer?.sample_qty}
                                            </p>
                                        ) : null
                                    }
                                    {
                                        product.offer?.gift_item_code ? (
                                            <p className="text-xs text-gray-600 mb-1 mt-1 p-2 bg-gradient-to-r from-green-100 to-green-300 rounded-lg shadow-md">
                                                <span
                                                    className="font-medium text-gray-700">Gift Item:</span> {product.offer?.gift_product_name} |
                                                Qty: {product.offer?.gift_item_qty}
                                            </p>
                                        ) : null
                                    }
                                    {
                                        product.offer?.additional_discount_amount ? (
                                            <p className="text-xs text-gray-600 mb-1 p-2 bg-gradient-to-r from-red-100 to-red-300 rounded-lg shadow-md">
                                                <span
                                                    className="font-medium text-gray-700">Additional Discount:</span> ৳{product.offer?.additional_discount_amount}
                                            </p>
                                        ) : null
                                    }
                                </div>
            ...
            {isEditPopupOpen && stockData && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-6 rounded-md shadow-lg w-96">
                        <h3 className="text-lg font-semibold mb-4 text-gray-800">Edit Quantity</h3>
                        <h3 className="text-sm font-semibold text-gray-800 mb-1 truncate">
                            {selectedProduct?.product_name}
                        </h3>

                        <div className="mb-4">
                            <label className="block text-gray-700">Available Stock: {stockData.stock}</label>
                            <label className="block text-gray-700">Stock per Unit: {stockData.stock_per_pis}</label>
                        </div>
